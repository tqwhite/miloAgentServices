 ...
 * 4. Named SQL query generation (getSql)
...
const moduleFunction =
	...
	({ baseMappingProcess, pwHash }) => {
		...
		// 2. Use <!tokenName!> for parameter substitution
		// 3. Access points call: dataMapping['mapper-name'].getSql('queryName', {params})
		// 4. qtTemplateReplace handles token→value substitution
		// 
		// STANDARD TOKENS:
		// - <!tableName!> - Replaced by database abstraction layer
		// - <!fieldName!> - Replaced with actual field values for WHERE clauses
		// 
		...
		// 2. Use token substitution for dynamic values
		...
		const getSql = (queryName, replaceObject = {}) => {
			...
			// Use qtTemplateReplace for token substitution (preferred over manual replacement)
